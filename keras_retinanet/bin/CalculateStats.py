...
def load_image(image_path, seqs):
    ...
    image_arrays = [np.asarray(Image.open(path).convert('L'))
    ...
    while not all_done:
        ...
        for c in range(len(combo_inds)):
            if overlap_areas[c] > thresh:
                ...
                new_poly = polys[cur_combo[0]].union(
                    polys[cur_combo[1]]).envelope
...
def CalculateStats(image, true_coords, bthresh=.5, othresh=.5, save=False, savepath=None):
    ...
    pred_polys = [GetPolyFromCoords(coords) for coords, score in zip(
        boxes[0], scores[0]) if score > bthresh]
    ...
    if savepath is not None:
        ...
        DisplayImageWithBboxes(
            draw, true_coords, pred_coords, savepath=savepath)

    return tp, fp, fn, tot


def CalculateStatsOnFold(fold):
    keras.backend.tensorflow_backend.set_session(get_session())

    # paths and parameters
    val_csv = 'C:\\Users\\jmj136.UWHIS\\Documents\\keras-retinanet\\keras_retinanet\\hcc_retinadata_val_fold{}.csv'.format(
        fold)
    model_path = os.path.join('C:\\Users', 'jmj136.UWHIS', 'Documents', 'keras-retinanet',
                              'keras_retinanet', 'snapshots_fold{}'.format(fold), 'best_weights.h5')
    image_output_dir = os.path.join(
        'C:\\Users', 'jmj136.UWHIS', 'Documents', 'keras-retinanet', 'OutputImages_fold{}'.format(fold))
    ...
    all_true_coords = [(int(d[1]), int(d[2]), int(d[3]), int(d[4]))
                       for d in data]
    ...
    for image_path in tqdm(unq_file_paths):
        ...
        tp, fp, fn, tot = CalculateStats(
            image, cur_true_coords, bthresh=.45, othresh=.5, save=True)
        true_pos.append(tp)
        false_pos.append(fp)
        false_neg.append(fn)
        total.append(tot)
    ...
    with open(tfile, "w") as text_file:
        ...
        text_file.write(
            'False positives per image: {:.02f}'.format(fp/len(false_pos)))

    print('Results written to {}'.format(tfile))


if __name__ == '__main__':
    CalculateStatsOnFold(0)
